...
- **Add / Edit form** — clicking a task opens it directly for editing, with
  **Save**, **Complete**, and **Delete** (with confirmation) actions. The form
  ...
  specific date/time), description, and target list (defaults to the sole
  selected list, or the last-used list when multiple or no lists are selected).
  Setting a task's first due date adds an at-due reminder by default. Reminders
  are stored as standard iCalendar `VALARM` components.
